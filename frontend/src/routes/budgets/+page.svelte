<script>
	...
	let inbudgets = null
    onMount(async () => {
		const budgetsResult = (await (await fetch(`/api/history/budget?len=1&month=0&year=1`)).json()).map(item => {
		...
		const inbudgetsResult = (await (await fetch(`/api/history/inbudget?len=1&month=0&year=1`)).json()).map(item => {
			item.date = `${new Date(item.date).getMonth()+1}-${new Date(item.date).getFullYear()}`;
			return item
		})[0]
