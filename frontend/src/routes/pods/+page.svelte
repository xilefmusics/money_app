<script>
    ...
    onMount(async () => {
		...
		pods = Object.keys(podHistory[podHistory.length-1]).filter(key => key !== 'date').map((key) => ({
			'name': key,
			'amount': podHistory[podHistory.length-1][key].value,
		}));
		datasets = pods.map(item => item.name).map(name => ({
			label: name,
...
<Chart
	type="line"
	data={{
		labels: podHistory.map(item => `${new Date(item.date).getMonth()+1}-${new Date(item.date).getFullYear()}`),
		datasets: datasets,
	}}
/>
<List
